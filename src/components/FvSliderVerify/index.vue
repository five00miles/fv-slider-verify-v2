        ...
        <div class="verify">
          <div class="verify--canvas" ref="canvas" :style="{ width: `${width}px`, height: `${height}px` }">
            ...
            <canvas
              ...
              class="block-canvas"
              ref="blockCanvas"
              @mousedown.prevent="(e) => drag(e, 'blockCanvas', 'circle')"
              @touchstart="
                (e) => {
                  terminal = 'mobile';
                  drag(e, 'blockCanvas', 'circle');
                }
              "
            ></canvas>
          </div>
          ...
          <div class="verify--slider">
            <div
              class="circle"
              ref="circle"
              @mousedown.prevent="(e) => drag(e, 'circle', 'blockCanvas')"
              @touchstart="
                (e) => {
                  terminal = 'mobile';
                  drag(e, 'circle', 'blockCanvas');
                }
              "
            >
...
export default {
  ...
  methods: {
    initCanvas() {
      ...
      const bgRandom = random(0, this.imgsName.length > 0 ? this.imgsName.length : 2);
      const img = new Image();
      img.crossOrigin = 'Anonymous';
      if (this.imgType === 'local') {
        if (this.imgsName.length > 0) {
          img.src = `images/${this.imgsName[bgRandom]}`;
        } else {
          img.src = require(`./images/${bgRandom}.png`);
        }
      } else {
        img.src = this.imgsPath + `${this.imgsName[bgRandom]}`;
      }
      img.onerror = () => {
